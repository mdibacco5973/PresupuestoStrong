...
const NavLinks = ({ className = "", setIsMobileMenuOpen }: { className?: string, setIsMobileMenuOpen: (val: boolean) => void }) => (
  <>
    <Link
      href="/"
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <nav className="hidden md:flex items-center space-x-6 text-sm font-medium ml-6">
            <NavLinks className="text-muted-foreground" setIsMobileMenuOpen={setIsMobileMenuOpen} />
          </nav>
        </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden border-t bg-background p-4 space-y-4 flex flex-col">
          <NavLinks className="text-foreground text-base py-2 border-b last:border-0" setIsMobileMenuOpen={setIsMobileMenuOpen} />
